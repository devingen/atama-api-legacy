...
type ScoreMap map[uint16]uint16
type ScoreMapMap map[uint16]ScoreMap
type HashIDMap map[string]uint16
	...
	for _, firstItem := range items1 {
		...
		for _, secondItem := range items2 {
			...
			if score > 0 {
				...
				if !hasHash {
					secondItemIdHash = util.SequentialHashUInt16()
		...
		if !hasHash {
			firstItemIdHash = util.SequentialHashUInt16()
...
func getFirstAvailableMatchInMap(pivotScoreMap ScoreMap, occupationMap PairMap) (uint16, uint16) {

	var maxScore uint16
	var itemWithMaxScore uint16
