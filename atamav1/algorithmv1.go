...
func GenerateScoreMap(
	...
	for _, firstItem := range items1 {
		...
		for _, secondItem := range items2 {
			...
			if score > 0 {
				scoreMap[secondItem.GetID()] = score
			}
		}

		scoreMapMap[firstItem.GetID()] = scoreMap
	}
	return scoreMapMap, hashIDMap
}

func getFirstAvailableMatchInMap(pivotScoreMap ScoreMap, occupationMap PairMap) (string, float64) {

	var maxScore float64
	var itemWithMaxScore string
